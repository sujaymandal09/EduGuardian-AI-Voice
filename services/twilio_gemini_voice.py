...
class TwoWayAIVoiceService:
    ...
    def _get_chat(self, student_data: dict):
        """Create AI chat with student context - English."""
        return self._genai_client.chats.create(
            model="gemini-2.5-flash",
            config={
                "system_instruction": f"""
                You are Priya, a professional and caring teacher from {self._school}.
    ...
    def make_call(self, payload: CallPayload) -> NotificationResult:
        ...
        student_data = {
            ...
            'attendance_pct': getattr(payload, 'attendance_pct', None),
            'attendance_total': getattr(payload, 'attendance_total', None),
            'attendance_attended': getattr(payload, 'attendance_attended', None),
            'consecutive_absences': getattr(payload, 'consecutive_absences', None),
            'performance_grade': getattr(payload, 'performance_grade', None),
            'behavior_incidents': getattr(payload, 'behavior_incidents', None),
        }
...
class TwoWayDemoService:
    ...
    def make_call(self, payload: CallPayload) -> NotificationResult:
        ...
        print(f"  🤖 Priya: \"I hope I'm not disturbing you.\"")
        ...
        print(f"  🤖 Priya: \"There's still time to improve.\"")
        ...
        print(f"  🤖 Priya: \"Please bring your child's registration card.\"")
        print(f"  🤖 Priya: \"Is there anything else I can help with?\"\n")
        time.sleep(0.5)
        
        print(f"  👤 {p}: \"No, that's all. Thank you so much!\"\n")
        time.sleep(0.5)
        
        print(f"  🤖 Priya: \"It was wonderful speaking with you!\"")
